    ...
    match result {
        Ok(res) => {
            if let Some(playlist) = res.get("playlist") {
                let name = playlist.get("name").and_then(|n| n.as_str()).unwrap_or("Unknown");
                let description = playlist.get("description").and_then(|d| d.as_str()).unwrap_or("");
                let track_count = playlist.get("trackCount").and_then(|v| v.as_i64()).unwrap_or(0);
                ...
                if let Some(tracks) = playlist.get("tracks").and_then(|t| t.as_array()) {
                    ...
                    for track in tracks.iter().take(5) {
                        ...
                        let ar = track.get("ar").and_then(|a| a.as_array())
